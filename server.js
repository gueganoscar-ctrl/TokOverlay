...
app.post('/api/contact', async (req, res) => {
  ...
  if (!message || message.trim() === '') return res.status(400).json({ error: "Le message ne peut pas être vide." });
  ...
  try {
    await resend.emails.send({
      ...
      html: `<div style="font-family: Arial; padding: 20px;"><h2>Nouveau retour (${type})</h2><p><strong>De:</strong> @${user.pseudo}</p><p>${message}</p></div>`
    });
...
app.post('/api/update-profile', async (req, res) => {
  ...
  try {
    ...
    if (nvApiKey !== "") updateData.apiKey = nvApiKey;

    const conflict = await db.collection('users').findOne({ pseudo: nvPseudo, email: { $ne: req.session.user.email } });
    if (conflict) return res.status(409).json({ error: "Ce pseudo est déjà utilisé par un autre compte." });

    await db.collection('users').updateOne({ email: req.session.user.email }, { $set: updateData });
...
app.post('/logout', (req, res) => {
  req.session.destroy(() => { res.clearCookie('__Host-tokoverlay'); res.json({ success: true }); });
});

app.get('/logout', (req, res) => {
  req.session.destroy(() => { res.clearCookie('__Host-tokoverlay'); res.redirect('/'); });
});
...
app.get('/api/admin/stats-globales', async (req, res) => {
  ...
  try {
    ...
    const resultat = streamers.map(s => {
      ...
      return {
        pseudo: s.pseudo, email: s.email, totalDiamantsGlobal: s.totalDiamantsGlobal || 0,
        diamantsSessionActuelle, enLigne: !!isOnline
      };
    });
...
function demarrerEcouteLive(pseudo, apiKey) {
  ...
  const data = {
    connection, closed: false, historySaved: false, refreshTimer: null,
    ...
    elimination: { actif: false, cout: 1, intervalle: 5, giftImage: '🌹', locked: false, eliminationEnCours: false, totalCoins: 0, places: [], timer: null },
  ...
  connection.on('gift', (d = {}) => {
    ...
    if (ELIGIBLE_GIFTS.includes(giftName)) {
      if (!waitingUsers.has(`${pseudo}_${username}`)) io.to(`streamer:${pseudo}`).emit('vip_alert', { username, giftName });
  ...
  connection.on('chat', (d = {}) => {
    ...
    if (waitingUsers.has(userKey)) {
      if (Date.now() <= waitingUsers.get(userKey)) io.to(`streamer:${pseudo}`).emit('roblox_pseudo', { username, message });
      waitingUsers.delete(userKey);
    }

    io.to(`streamer:${pseudo}`).emit('chatEnDirect', { nickname, avatar, message });

    if (data.enchere && data.enchere.dons[id]) data.enchere.dons[id].dernierMessageChat = message;
...
function etatCoffrePublic(pseudo) {
  ...
  return {
    actif: coffre.actif, longueur: coffre.secret.length, devoiles: coffre.devoiles,
    caracteres: coffre.secret.split('').map((char, index) => coffre.devoiles[index] ? char : '_'),
    recompense: coffre.recompense, gagnant: coffre.gagnant,
    dernierMessageGagnant: coffre.dernierMessageGagnant, secretComplet: coffre.gagnant ? coffre.secret : null
  };
}
...
function sauvegarderHistoriqueLive(pseudo, customData = null) {
  ...
  db.collection('historique_lives').insertOne({
    pseudo, debut: data.debutLive, fin: new Date(), totalDiamants, totalLikes,
...
function demarrerEnchere(pseudo, dureeSecondes, snipeSecondes, miseMinimale) {
  ...
  const enchere = {
    actif: true, phase: 'timer', snipeMs: snipeSecondes * 1000, miseMinimale: miseMinimale || 0,
    finTimestamp: Date.now() + dureeSecondes * 1000, dons: Object.create(null), minuteur: null, totalDiamantsEnchere: 0
  };
...
function etatObjectif(pseudo) {
  ...
  return { label: data.objectif.label, metrique: data.objectif.metrique, cible: data.objectif.cible, valeurActuelle };
}
...
function etatEnchere(pseudo) {
  ...
  return {
    phase: enchere.phase, tempsRestant: Math.max(enchere.finTimestamp - Date.now(), 0),
    snipeMs: enchere.snipeMs, miseMinimale: enchere.miseMinimale,
...
function programmerTransitionOuFin(pseudo, enchere) {
  ...
  if (enchere.minuteur) { clearTimeout(enchere.minuteur); enchere.minuteur = null; }
...
function terminerEnchere(pseudo) {
  ...
  if (donsValides.length >= 2 && donsValides[0].coins === donsValides[1].coins) {
    enchere.phase = 'timer'; enchere.finTimestamp = Date.now() + 30000;
  ...
  const data = connexionsActives[pseudo];
  if (data) { data.derniereGagnantId = gagnant?.id || null; data.vouchFait = false; }

  if (db) {
    db.collection('historique_encheres').insertOne({
      pseudo, date: new Date(),
  ...
  io.to(`streamer:${pseudo}`).emit('enchereTerminee', { gagnant, classement: donsValides.slice(0, 3), totalDiamantsEnchere: enchere.totalDiamantsEnchere });
}
...
io.on('connection', socket => {
  ...
  // SOCKETS POUR L'ÉLIMINATION (CREATION ET MODIFICATION DYNAMIQUE)
  ...
  // METTRE A JOUR LE TEMPS ET LE PRIX EN TEMPS REEL SANS RESET
  socket.on('updateEliminationSettings', (payload = {}) => {
    try {
      ...
      if (elim && elim.actif) {
        ...
        // Si le minuteur de mort tourne déjà, on le relance avec le nouveau temps
  ...
  socket.on('configurerRoue', (payload = {}) => {
    try {
      ...
      const data = connexionsActives[pseudoNettoye];
      if (data) {
        if (Array.isArray(options)) data.roue.options = options.slice(0, 20).map(opt => safeText(opt).slice(0, 80)).filter(Boolean);
        if (montantMin !== undefined) data.roue.montantMin = positiveInteger(montantMin, 1);
      }
    } catch {}
  });
  ...
  socket.on('demarrerEnchere', (payload = {}) => {
    try {
      ...
      const pseudoNettoye = normalizePseudo(pseudo);
      if (!canManage(user, pseudoNettoye)) return;
      if (connexionsActives[pseudoNettoye]) demarrerEnchere(pseudoNettoye, strictInteger(dureeSecondes, { min: 5, max: 86400 }), strictInteger(snipeSecondes, { min: 1, max: 3600 }), strictInteger(miseMinimale, { min: 0, max: 1000000 }));
    } catch {}
  });
  ...
  socket.on('definirObjectif', (payload = {}) => {
    try {
      ...
      data.objectif = { cible: strictInteger(cible, { min: 1, max: 10000000 }), metrique: metrique === 'likes' ? 'likes' : 'diamants', label: safeText(label, 'Objectif du live').slice(0, 60) };
  ...
  socket.on('configurerCoffre', (payload = {}) => {
    try {
      ...
      data.coffre = { actif: true, secret: cleanSecret, devoiles: new Array(cleanSecret.length).fill(false), recompense: safeText(recompense, '').slice(0, 50), gagnant: null, dernierMessageGagnant: '' };
